...
if __name__ == '__main__':
    # Getting the initial variables, and starting a while loop which runs the market clearing on the blockchain and
    # on python. A difficult thing is to retrieve the data
    offers_blockchain, bids_blockchain, offers_db, bids_db, user_infos_blockchain, user_infos_db, id_meters_blockchain, id_meters_db, config, quality_energy, price_index, db_obj = test_utils.setUp_test(
        generate_bids_offer, timeout=timeout_blockchain)
    ...
    for t_sub in t_submissions:
        ...
        for ob in pd.concat([offers_blockchain_df, bids_blockchain_df]).iterrows():
            blockchain_utils.functions.pushOfferOrBid(tuple(ob[1].values), ob[1][
                list(ob[1].keys()).index(db_param.TYPE_POSITION)] == 'offer', True).transact(
        ...
        market_results_python_t_sub = \
        db_obj.get_results_market_ex_ante(ts_delivery_first=ts_delivery_first, ts_delivery_last=ts_delivery_last)[0]
