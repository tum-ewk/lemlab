...
def test_sorting():
    ...
    start = time.time()
    sorted_offers_db = open_offers_db.sort_values(by=[db_param.PRICE_ENERGY, db_param.QUALITY_ENERGY, db_param.QTY_ENERGY],
                                             ascending=[True, False, False])
    end = time.time()
    print("offers_python_sorted done in " + str(end - start) + "seconds")
    start = time.time()
    sorted_bids_db = open_bids_db.sort_values(by=[db_param.PRICE_ENERGY, db_param.QUALITY_ENERGY, db_param.QTY_ENERGY], ascending=[True, False, False])
    end = time.time()
    print("bids_python_sorted done in " + str(end - start) + "seconds")
    sorted_offers_db_list, sorted_bids_db_list = [tuple(x) for x in sorted_offers_db.values.tolist()], [tuple(x) for x in sorted_bids_db.values.tolist()]

    blockchain_utils.setUpBlockchain(contract_name="Sorting")

    start = time.time()
    offers_blockchain_sorted_three_keys = blockchain_utils.functions.insertionSortOffersBidsPrice_Quality(open_offers_blockchain, True, False, True,
                                                                             False).call()
    end = time.time()
    print("offers_blockchain_sorted done in " + str(end - start) + "seconds")

    start = time.time()
    bids_blockchain_sorted_three_keys = blockchain_utils.functions.insertionSortOffersBidsPrice_Quality(open_bids_blockchain, True, False, True, False).call()
    end = time.time()
